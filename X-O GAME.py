...
show () 
z=int(input("if you want play with commputer please Enter [1] or press any digit to play with other player :"))
if z==1:
   print ("START GAME")  
   while True:  
     A = int(input("Select a Position: "))
     i+=1
     if A>9 or A<0 :
          ...
          if board[A] !="X" and board[A] !="O" :
              ...
              while True:
                    i+=1 
                    random.seed()
                    oppo = random.randint(1,9)

                     
                    if board[oppo] !="O" and board[oppo] !="X":
                       board[oppo] = "O"
                       if winCheck("O") == True:
                           print("you lose ")
                           show()
